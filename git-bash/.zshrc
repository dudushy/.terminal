...
ZSH_THEME="headline"
# "dracula", "jispwoso", "jonathan", "xiong-chiamiov-plus", "headline"
...
plugins=(git branch colorize composer encode64 git-auto-fetch git-prompt ionic gradle npm nvm pip pm2 vscode zsh-syntax-highlighting zsh-autosuggestions history jsontools dirhistory sdk timer gh git-extras git-lfs gitfast sublime)
...
# Function to check for .nvmrc file and install the specified Node.js version
nvm_auto_use() {
  if [ -f .nvmrc ]; then
    nvm_version=$(cat .nvmrc | tr -d '\r')
...
# Zsh hook to run the function when changing directories
